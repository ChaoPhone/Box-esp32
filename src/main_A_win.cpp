...
// =============================================================
//  A 板 (main_A_win) — USB 连 PC 的桥接板
// -------------------------------------------------------------
//  职责：ESP-NOW 接收 B 板的协议行 → 过滤 → 转发 USB 串口
//        PC 下行命令 → ESP-NOW 转发给 B 板
...
void setup() {
  ...
  espnowSetup();
  gBoard = boardLetter();
  Serial.printf("LOG,[%c] A-board ready, MAC %s\n", gBoard, WiFi.macAddress().c_str());
...
void loop() {
  ...
  while (xQueueReceive(rxQueue, &m, 0)) {
    if (!loggedAlive) {
      loggedAlive = true;
      Serial.printf("LOG,[%c] link alive, first frame from B\n", gBoard);
    }
    forwardToPc(m.data);
  }
