...
// =============================================================
//  B 板 (main_B_esp) — 箱子端，MPU6050 IMU + ESP-NOW 无线
// -------------------------------------------------------------
//  职责：IMU 采样 → Mahony 融合 → 推动/倾斜/抬举检测
//        ESP-NOW 发送 EVT/HEARTBEAT/IMUQ 到 A 板
//        接收 A 板命令（HAPTIC→电机, STATE→LED）
//
//  A 板 MAC: E8:3D:C1:F2:C7:B8    B 板 MAC: E8:3D:C1:FA:7A:0C
//
//  构建：pio run -e b_esp -t upload
// =============================================================

// ---------------- 协议常量 ----------------
static const char *FIRMWARE_VERSION = "0.9.0-besp";
...
static void espnowSetup() {
  ...
  memcpy(PEER_ADDR, MAC_A, 6);  // B 的对端是 A
  gPeerKnown = true;
...
void setup() {
  ...
  espnowSetup();
  gBoard = boardLetter();
  Serial.printf("LOG,[%c] B-board ready, MAC %s, IMU=%d\n", gBoard, WiFi.macAddress().c_str(), imuPresent);
