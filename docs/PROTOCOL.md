...
### 3.1 EVT action 名清单（v0.8.2-tilt）
...
**方向基准**：四元数倾斜补偿 + 去 yaw 后的**机体航向系**——前后左右锚定盒体自身轴
（硬件安装约定见 `MPU6050_WIRING.md` §5），与盒子放置姿态、yaw 漂移无关。
若安装方向与预期不符，用下行 `ORIENT` 命令软件校正，无需重烧。

**判定链**（0.8.1）：高通去 DC → 过阈值开峰值捕获窗（取窗内最强样本判向）→
回摆抑制（推完回位的反向脉冲丢弃）→ 再武装迟滞（信号须回落安静后才允许下一次触发）。
**语义保证：一次物理推动 = 恰好一条 EVT**，宿主侧无需再做去抖/合并。

当前参数（固件常量，调参需重烧）：

| 参数 | 值 | 含义 |
|---|---|---|
| `PUSH_THRESH` | 0.25 g | 触发阈值 |
| `PUSH_PEAK_WIN_MS` | 120 ms | 峰值捕获窗（也是 EVT 固有延迟） |
| `PUSH_DEBOUNCE_MS` | 600 ms | 触发后硬锁定 |
| `PUSH_RETURN_SUPPRESS_MS` | 900 ms | 回摆（反向脉冲）抑制窗 |
| `PUSH_SAME_DIR_SUPPRESS_MS` | 100 ms | 同向重复抑制窗 |
| `PUSH_REARM_THRESH` / `QUIET_MS` | 0.15 g / 200 ms | 复位迟滞：回落至 0.15g 以下持续 200ms 才能再触发 |
| `PUSH_WARMUP_MS` | 1500 ms | 上电/CALIBRATE 后静默期，期间不产生 EVT |

连续推动的最短间隔 ≈ 600ms（锁定）+ 回落安静 200ms，实际手感约 **1 次/秒**。
...
#### 倾斜 / 抬举（姿态角 + 垂直加速度检测，✅ 已实现）

| action | 含义 | 触发条件 |
|--------|------|----------|
| `TILT_FORWARD` | 向前倾斜 | 加速度 Y 方向，倾角 > 45° 且比率最大 |
| `TILT_BACKWARD` | 向后倾斜 | 加速度 Y 反方向，倾角 > 45° 且比率最大 |
| `TILT_LEFT` | 向左倾斜 | 加速度 X 反方向，倾角 > 45° 且比率最大 |
| `TILT_RIGHT` | 向右倾斜 | 加速度 X 方向，倾角 > 45° 且比率最大 |
